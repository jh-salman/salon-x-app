...
export const MOCK_OVERLAY_PAST_VISITS: Record<
  'info' | 'consultation' | 'services' | 'maintain' | 'notes',
  string
> = {
  info:
    'Earlier profile intel (sample)\n\n' +
    '• Preferences and referral history will sync from TAG / AFTERBURNER.\n\n' +
    'Replace with live CRM when connected.',
  consultation: '',
  services:
...
export const MOCK_CLIENT_DETAILS: Record<string, ClientDetails> = Object.fromEntries(
  DB_CLIENTS.map((c) => {
    ...
    const aptCount = DB_APPOINTMENTS.filter((a) => a.clientId === c.id).length;
    const detail: ClientDetails = {
      id: c.id,
      clientName: c.fullName,
      phone: c.phone ?? undefined,
      visitCount: aptCount + (c.id === 'client-jon-klein' ? 11 : 0),
      showReferralBadge: c.id === 'client-mia-chen',
      aiConsultationBrief:
        apt?.bookingNotes?.trim() ||
        `Pre-arrival brief (Ghost Notes): ${c.notes?.trim() || '—'}`,
