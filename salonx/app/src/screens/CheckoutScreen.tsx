...
export default function CheckoutScreen({ detailId, clientDetails }: Props) {
  ...
  // Prefill-only: if a Visit Plan exists for this appointment, use it as the UI source.
  const { resolvedSelected } = useVisitPlanStorage(detailId, { services, recommendations });
  const effectiveServices = resolvedSelected.services.length > 0 ? resolvedSelected.services : services;
  const effectiveRecommendations =
    resolvedSelected.recommendations.length > 0 ? resolvedSelected.recommendations : recommendations;

  const suggestedItems = useMemo(() => {
    const recs = Array.isArray(effectiveRecommendations) ? effectiveRecommendations : [];
    if (recs.length >= 2) {
  ...
  const servicesTotal = useMemo(
    () => effectiveServices.reduce((sum, s) => sum + (Number.isFinite(s.price) ? s.price : 0), 0),
    [effectiveServices]
  );
  ...
  const futureLine = `${format(futureDate, "M/d/yyyy")} — ${effectiveServices[0]?.name ?? "Service"}`;
